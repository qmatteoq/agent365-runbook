...
citing what they retrieved. None of them contains a single line of Agent 365 observability code. The
.NET and Python ones do ship the Microsoft Entra sign-in the OBO path depends on, but it stays
dormant until the settings are filled in, so you can run them anonymously before anything is
registered and switch the sign-in on in Phase 1 once there's a blueprint to point it at.
