...
**Primary Tools**: Agent 365 Skills, Agent 365 CLI, Microsoft Entra ID, OpenTelemetry, Work IQ MCP servers
...
Onboarding to Agent 365 fixes all four. This runbook shows you how, on a **user-driven web app**
where a person signs in and drives the agent.
...
If your agent runs on a schedule, reacts to events, or acts when nobody is watching, this is the
wrong scenario — you want
[AI Teammate — Agent Identity](../AI-Teammate-Agent-Identity/) instead. If it's hosted in Teams
rather than a web page, you want
[Teams Agent — Custom Engine OBO](../Teams-Agent-Custom-Engine-OBO/).
...
- Adding the Entra sign-in that produces the user assertion the OBO chain needs
...
- Adding Work IQ MCP servers (Mail, Calendar and others)
...
Three are provided, functionally identical:
...
citing what they retrieved. None of them contains a single line of Agent 365 code, and none of them
signs anybody in — the sign-in depends on the blueprint that doesn't exist yet, so the runbook adds
it in Phase 1 once there's something to point it at.
