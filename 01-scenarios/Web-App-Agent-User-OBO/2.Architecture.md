...
Three layers. The important thing to notice is that **two different identities** are involved on
...
| --- | --- | --- |
| Web app | Blazor Server / FastAPI | Hosts the chat UI and the agent loop |
...
| A365 exporter | `Microsoft.OpenTelemetry` / `microsoft-opentelemetry` | Ships spans to Agent 365 |
...
So: **activity in Defender but not the admin center means your `invoke_agent` span is the problem**
— most often its caller identity — not your export plumbing. Source:
[observability concepts — where your data shows up](https://learn.microsoft.com/microsoft-agent-365/developer/observability-concepts#where-your-data-shows-up).

Equally important: **`HTTP 200` from the exporter is not proof of ingestion.** A successful post
with an empty `partialSuccess` can still be dropped downstream — for example when no user in the
tenant holds an Agent 365 licence.
