...
> ⚠️ **The skill's reference documentation says `UseMicrosoftOpenTelemetry` registers this for you**, but it's actually not correct, this is why you must register it manually.
...
From now on, instead, these steps are listed as a reference, so that you can better understand what happens behind the scene. You don't have to follow them manually, this code will be integrated for you by the skill.
...
The A365 Tooling SDK switches to `DevMcpTokenProvider` — which expects you to paste a bearer token in by hand — whenever `ASPNETCORE_ENVIRONMENT` is exactly `"Development"`. That's fine for quick experiments, but not very useful for exercising the real agentic path. So we run locally as Production instead. But `CreateBuilder` only loads user-secrets in Development, so running as Production means the connection is built with an empty `ClientSecret` and every single turn fails with `Failed to create authentication provider for connection name ''`.
...
| `Failed to create authentication provider for connection name ''` (.NET) | User-secrets aren't loaded in Production — Step 2-9 |
