...
> ⚠️ **Before anything else: this scenario needs a Frontier tenant.** The AI Teammate capability is still in preview, and it is gated on your tenant being enrolled in the [Microsoft 365 Copilot Frontier program](https://www.microsoft.com/en-us/microsoft-365-copilot/frontier-program). Agent instances draw on **Microsoft 365 Frontier for Autopilots** licences, which is a pool your tenant only has if it's enrolled, and it's a finite one, so it's worth knowing how many you have before you start. Outside a Frontier tenant you can still work through Phase 0 locally, but Phase 1 has nowhere to publish to and everything after it depends on Phase 1. Check your enrolment first rather than discovering it halfway through an upload wizard.
...
They are the same agent, answering the same questions from the same Microsoft Learn MCP server. Pick one and follow it through. Where they differ, the step says so explicitly and shows each one.
...
On **Python** and on **Node.js**, in `.env` (copy it from the committed `.env.example`; `.env` itself is gitignored):
...
The two stacks read the same key names, so the block above is the same for both.
...
Now pick how the agent authenticates to Azure OpenAI. All three starting points support two paths and choose between them at startup with a simple rule: if an API key is configured, they use key auth; if not, they use Entra credentials.
...
# Path B, Python and Node.js — set it in .env, which is gitignored
# AZURE_OPENAI_API_KEY=<key>
```
### Step 0-2. Check the Agents SDK configuration

There are no bot credentials to fill in, that's the whole point of this scenario, but the three stacks still need slightly different treatment before they'll start.
...
On **Node.js**, the same three connection keys are there, spelled the way the Node SDK parses them, plus one that looks unrelated and isn't:
...
Whichever you picked, the channel endpoint is at `/api/messages` and there's a plain liveness string at `GET /`.

Note the ports: 3980 for .NET, 3981 for Python and 3982 for Node.js. The Teams starting points in this repository use 3978 and 3979, so every agent in the repository can run side by side without colliding.
...
> The Playground works with no configuration because the starting point runs in anonymous mode, `TokenValidation.Audiences` is empty on .NET, `ANONYMOUS_ALLOWED` is `true` on Python, and on Node.js the client id is blank while `NODE_ENV` is `development`, so unauthenticated activities are accepted. The Agent 365 CLI writes the real connection settings during Phase 1. This is also the mode the warning in the introduction was about: convenient locally, not something to leave exposed on a public URL.
...
> **Don't take that command's own verdict at face value either.** `setup permissions bot` opens a browser for consent and gives you three minutes to complete it; if you're slow, it times out, reports failure, and then asks a *second* consent question you weren't expecting. The awkward part is that the consent may well have gone through in the browser before the timeout, so a reported failure doesn't reliably mean nothing happened. Check what actually landed rather than trusting the exit message:
> ```bash
> a365 query-entra blueprint-scopes
> ```
> That lists the permissions currently on the blueprint. If `Agent365.Observability.OtelWrite` is there, you're done regardless of what the previous command said, and re-running it "to be safe" costs you another client secret.
...
The wrinkle is that publishing and activating are two different things that the admin center presents together, so it's easy to come away thinking you've done both when you've only done one. **Publishing** makes the blueprint available to users, so they can ask for an instance of it. **Activating** decides who is actually allowed to create one. An agent can be published and not activated, and that combination is the one that quietly stalls everything downstream.
...
With that done, the agent appears in Teams under **Apps** → **Agents for your team**. This is not instant, give it a few minutes before deciding something has gone wrong. Its store entry looks like this:
...
The button reads **Create instance**, not *request*, which rather undersells what happens next, because unless you are a tenant administrator yourself, pressing it creates a request rather than an instance. That request lands here, for an administrator to approve:
...
> ⚠️ **The skill's reference documentation says `UseMicrosoftOpenTelemetry` registers this for you**, but it's actually not correct, this is why you must register it manually. If you want to see it for yourself rather than take our word for it, count the service descriptors before and after the call: on 1.0.7 it adds fourteen, and `IExporterTokenCache<AgenticTokenStruct>` is not among them. The quicker demonstration is to simply leave this step out. The host then refuses to start, with `System.ArgumentNullException: Agent365 exporter requires a TokenResolver or ContextualTokenResolver.` That message also hints at a second option the documentation doesn't mention at all, `o.Agent365.ContextualTokenResolver`, which receives the resolved identity alongside the tenant. We don't need it here, but it's there if you ever have to resolve tokens per-identity rather than per-call.
...
On **Node.js** the deferred cache is back, and this is the one place in the phase where Node has the best of the three. Install the distro first:
...
That leaves a genuine three-way split worth holding onto, because it explains why the next few steps read so differently depending on which stack you picked:

| Stack | How the exporter gets its token |
| --- | --- |
| .NET | Register the *means* to fetch one, and the cache resolves it on demand |
...
There's a lot in that block, so let's pull out the two decisions that are easy to get wrong.
...
One thing worth saying explicitly, because it's the sort of gap people try to fill with a custom span processor: on all three stacks the identity gets onto the span by itself. You never copy it there. The SDK reads the turn's baggage when it builds each span, so anything opened inside the scope from Step 2-6, the invoke span, the inference span underneath it, the tool spans underneath that, comes out already carrying the agent and tenant. You can watch it happen with the console exporter: open a scope with an empty `AgentDetails` and the agent id still arrives on the span, because it came from baggage rather than from what you passed in. What matters is that the work happens *inside* the scope. Nothing else is required, and there's nothing to register.
...
**Node.js** works the same way as Python, and for the same reason: the distro's LangChain instrumentation produces the inference and tool spans once it has patched the library. The failure mode is the one from Step 2-3, an import that ran before `src/observability.ts` did, so if the `chat` spans are missing this step isn't where to look.
...
On **Node.js** the accessors carry the same names in camel case, and they behave identically:
...
`isAgenticRequest()` checks the recipient's role for `agenticUser` or `agenticIdentity`, and `getAgenticInstanceId()` returns `recipient.agenticAppId` only when that check passes, so the guard is built in rather than something you have to remember. `getAgenticTenantId()` falls back to the conversation's tenant when the recipient carries none, which is the small convenience that saves you reading it out of the environment. The Playground caveat from the Python paragraph applies here just as much: a Playground turn is not an agentic turn, so these all come back `undefined` and Phase 2 will look broken when it isn't.
...
On **Node.js** the builder is the same fluent chain, and `build()` again produces the scope rather than the builder being one itself. What differs is how the scope is entered: there is no `with` and no `using` in plain TypeScript, so the scope hands you a `run` method and everything that should inherit the baggage goes inside the callback:
...
`run` is generic over the callback's return type, so an `async` callback comes back as a promise you can await, and the baggage stays attached across the awaits inside it because the distro rides on Node's `AsyncLocalStorage`. The trap is the mirror image of Python's block boundary: return the promise without awaiting it *outside* `run` and the baggage is still attached, but await the model call outside the callback and it isn't. When in doubt, keep the whole turn inside.
...
There is one prerequisite for all of this that lives nowhere near the observability code, and it is the single easiest thing in this phase to get wrong. The deferred generator you just registered doesn't mint a token out of thin air: it exchanges the token the turn already holds, and the turn only holds one if the route that handled it was registered with an auto sign-in handler. If you registered the message route the plain way:
...
> This one is worth being stubborn about, because skipping it costs nothing visible. The agent still answers in Teams, `RegisterObservability` still succeeds, the cache still reports an entry, and every span is still built with the right identity. The only symptom is that no token ever appears and nothing is ever sent, and because the exchange is never even attempted, there is no error, no `AADSTS` code and no failed request anywhere in the logs to lead you back here.
...
On **Node.js** you also acquire during the turn, but you hand the work to the built-in cache rather than doing the exchange yourself:
...
Two arguments deserve more attention than their position suggests.

**Pass the observability scope explicitly.** Leave it out and the cache falls back to `api://9b975845-388f-4429-889e-eab1ef63949c/.default`, which is the right value, so this looks like a free choice. It isn't, quite: if you ever reach for `authorization.getToken` instead, that uses the *handler's* configured scopes, and the CLI writes Graph scopes into the handler. The agent instance has no Graph consent, so you get `AADSTS65001 ... consent_required` from a call that looks identical to the working one. Being explicit here keeps that distinction visible.

**The handler name is lowercase `agentic`.** That's the parameter's default, and it happens to match the middle segment of the key the CLI writes into `.env`. It's worth checking rather than assuming, because Python's runbook uses `AGENTIC` in upper case for the same handler and the two are not interchangeable. Get it wrong and the exchange finds no handler, which your `try`/`catch` then swallows into a log line.

What Node.js does *not* need is the auto sign-in registration that .NET does. `AgenticAuthorization.token()` asks the connection for an agentic user token directly, with no "must already have signed in" precondition, so `exchangeToken` reaches the handler from an ordinary message route. That also means Node avoids Python's silent-empty-token trap entirely. Wrap the call in a `try`/`catch` that logs and carries on, the same as the other two stacks.
...
The endpoint is the one place where the .NET advice above doesn't carry over. Python wants a `ServiceEndpoint`, and it wants a bare **hostname** rather than a URI, specifically `ServiceEndpoint(hostname=..., port=None)`. Passing the URI string directly is the natural mistake, and `InvokeAgentScopeDetails` is a plain dataclass that accepts it without complaint; the failure comes later, from inside the scope, as `AttributeError: 'str' object has no attribute 'hostname'`. The `.invalid` convention still holds, and for the same reason: the value is never resolved, so build it from the blueprint GUID rather than from a display name.
...
On **Node.js** the scope is neither a `using` block nor a context manager, so it is started, run and disposed by hand:
...
Three details are specific to Node and each one costs you an afternoon if you meet it cold. The request contract is exported as **`A365Request`** rather than `Request`, because `Request` is already a global in Node's own type definitions and importing a second one produces errors that point nowhere useful. The endpoint takes **`host`**, not `hostname` as Python does and not a URI as .NET does, though the `.invalid` convention and the reason for it are the same in all three. And the scope only becomes the *active* span inside `withActiveSpanAsync`, so work started outside that callback produces spans parented somewhere else, or nowhere at all.

The `finally` matters more here than the equivalent does elsewhere, because nothing disposes the scope for you. Miss it and the span is never ended, so it is never exported, and the turn simply doesn't appear.
...
There are three quirks to deal with before you can run this locally, and they share a single root cause: the framework treats the `Development` environment as a special case in ways that don't suit an agentic agent.
...
The third quirk only bites you on Path A, where you authenticate to Azure OpenAI with `DefaultAzureCredential` rather than an API key, and it is the one most likely to send you looking in the wrong place. The starting point decides whether to include Managed Identity in the credential chain like this:

```csharp
ExcludeManagedIdentityCredential = builder.Environment.IsDevelopment(),
```

Read together with everything above, the problem should be apparent: Managed Identity is excluded *only* in Development, and we have just spent this whole step arranging to run as Production. So the credential chain now starts by asking the Azure Instance Metadata Service for a token, and that service lives at `169.254.169.254`, an address that only answers on Azure infrastructure. On your laptop it answers nothing, and every model call fails with this:
...
What you see in Teams is far less helpful, just a generic apology about something going wrong, which points you at your tools or your model deployment rather than at credential selection. So we key the exclusion off whether we are genuinely hosted on Azure, instead of off the environment name:
...
On **Node.js** the same environment variables apply, and the distro is quiet unless you ask it to speak, so set the log level in the `a365` block or through the environment:

```powershell
$env:A365_OBSERVABILITY_LOG_LEVEL="info|warn|error"
```

The default is `none`, which is worth knowing before you conclude that nothing is happening: an agent exporting perfectly and an agent exporting nothing look identical at the default level. With logging on, a healthy run names the exporter and the endpoint, and the two lines to look for are a token cache hit and an export to a `agent365.svc.cloud.microsoft` URL carrying the agentic instance id. A line from `[AgenticTokenCache]` saying there is no cache entry for a key means Step 2-7 never ran for that turn, or ran with a different agent id than the span was built with.

On **.NET** the equivalent signals read differently, and they're worth setting out because two of them look like failures when they aren't. Raise the level on the exporter's own category to see any of this at all:
...
The skipped span is the ASP.NET request span. It's created before your baggage scope opens, so it carries no identity and there's nowhere to send it. That line will appear on every batch and it isn't a problem. The lines that actually matter are the last two. Check the agent id in them: that should be the agentic **instance** id, not the blueprint id from `appsettings.json`. And check that a `Sending chunk` line follows, with a `agent365.svc.cloud.microsoft` URL. The accompanying HTTP response should be a `200` or a `201`. If you have both, the whole chain from Step 2-5 through Step 2-7 is working and traces are genuinely leaving the machine.

> If instead you see `No token obtained. Skipping export for this identity.` immediately after a line saying a token was obtained for that same agent and tenant, the two lines really are contradicting each other, and the answer is almost always the auto sign-in registration from Step 2-7. The first line only reports that an identity group was matched, not that a token came back; the second reports the truth. Go back and check that your message route was registered with `autoSignInHandlers`, then look for the observability resource id `9b975845-388f-4429-889e-eab1ef63949c` in a `Trace`-level run. If it appears nowhere, no exchange was ever attempted, which is exactly what a missing auto sign-in handler looks like.
