...
The onboarding is genuinely different in each of the three, most of all in Phase 2, so every step that varies is organized the same way: a short introduction that explains what we're doing and why, and then three subsections, **.NET**, **Python** and **Node.js**, always in that order.
...
There's nothing to do. `TokenValidation:Audiences` is already empty, which is what puts the agent in anonymous mode, and there's deliberately no `Connections` section. `No connections found in configuration` in the startup logs is expected at this stage and not an error.
...
> **Don't skip the JWT middleware to allow anonymous calls instead.** It's the tempting shortcut and it doesn't work: the middleware is what attaches a `claims_identity` to the request, and the adapter reads that to decide whether the turn is anonymous. Skip it and the turn is treated as *authenticated*, the adapter tries to build a real user token client, and the request fails with `TENANT_ID is not set in the configuration`.
...
With that done, the agent appears in Teams or in the Agent Store in Copilot under **Apps** → **Agents for your team**. This is not instant, give it a few minutes before deciding something has gone wrong. Its store entry looks like this:
...
The lock is there because the writer is your turn and the reader is the exporter's flush thread. If that shape looks familiar, it's the same one the [web app OBO scenario](../Web-App-Agent-User-OBO/3.Runbook.md) uses. The deferred cache is a .NET convenience, not a requirement of the protocol.
...
Now we turn the distro on and point it at Agent 365. Three decisions are being made in the code below, and they're the same three whatever you're on, so they're worth naming before you read any of it.
...
**Hand over the getter from Step 2-2, not a token.** Nothing has fetched one yet, and that's fine: the exporter calls the resolver again on every flush, so by the time it matters, Step 2-7 will have filled the cache in.
...
There's a lot in that block, but the two decisions that are easy to get wrong are the ones named in the introduction: the console exporter sits alongside the Agent 365 one rather than replacing it, and `UseS2SEndpoint` stays at its default.

#### Python

The same three decisions are expressed as arguments to a single call:
...
Every span this agent exports carries a `gen_ai.agent.id`, and getting it wrong is the single most common mistake in this repository, because the correct answer is different in all three scenarios:
...
What differs is how you *enter* the scope, because each language has its own idea of what a scope even is.
...
This is where a real observability token finally reaches the storage we set up in Step 2-2, and it's the longest step in the phase because the three stacks genuinely diverge here.

They divide into two models. On .NET you register the *means* to obtain a token and nothing is fetched until the exporter asks. On Python and Node.js you acquire the token during the turn and put it away. The .NET model needs less code but fails later and more quietly; the other two fail immediately, which is inconvenient at exactly the moment it's most useful.

One thing is true of all three: wrap the call in a `try`/`catch` that logs and carries on. A lost trace should never cost the user their answer.
...
**The handler name is lowercase `agentic`.** That's the parameter's default, and it happens to match the middle segment of the key the CLI writes into `.env`. It's worth checking rather than assuming, because the Python handler above is spelled in upper case and the two are not interchangeable. Get it wrong and the exchange finds no handler, which your `try`/`catch` then swallows into a log line.

What Node.js does *not* need is the auto sign-in registration that .NET does. `AgenticAuthorization.token()` asks the connection for an agentic user token directly, with no "must already have signed in" precondition, so `exchangeToken` reaches the handler from an ordinary message route. That also means Node avoids Python's silent-empty-token trap entirely.
...
One detail is shared by all three stacks: build the endpoint from the blueprint GUID, under the reserved `.invalid` TLD. It's tempting to slugify the agent's display name into something readable instead, but free-form names contain characters that produce a URI parsing error, and the value is never resolved anyway, so readability buys you nothing.
...
What differs is how loudly each stack tells you.
...
Two of those lines look like failures and aren't. The skipped span is the ASP.NET request span: it's created before your baggage scope opens, so it carries no identity and there's nowhere to send it. That line appears on every batch. The lines that actually matter are the last two, the token and the chunk.
