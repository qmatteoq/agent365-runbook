...
The agent we're onboarding is hosted exactly like a classic custom engine agent: an ASP.NET Core app, the Microsoft 365 Agents SDK, an `AgentApplication` handling turns, a `/api/messages` endpoint receiving Bot Framework activities. All of that is unchanged. And yet there is no Azure Bot resource anywhere in this guide — we never create one, and the starting point has no `Connections` section to hold its credentials.
...
> **Don't leave `TenantId` out.** Without it, the credential hands back a token from whichever tenant you last signed in to — which may not be the one that owns the Azure OpenAI resource — and the service answers `HTTP 400 – Tenant provided in token does not match resource token`. It reads like a code bug and isn't one.

Now pick how the agent authenticates to Azure OpenAI. The starting point supports two paths and chooses between them at startup with a simple rule: if an API key is configured, it uses key auth; if not, it uses Entra credentials.

|  | **Path A — Entra credentials** *(default, recommended)* | **Path B — API key** |
| --- | --- | --- |
| What it actually uses | `DefaultAzureCredential` — your `az login` session locally, the assigned managed identity once hosted on Azure | A resource key, sent as a bearer secret |
| Extra configuration | None | The key, kept outside the committed `appsettings.json` |
...
# Path B — set the key outside the committed files
dotnet user-secrets set "AzureOpenAI:ApiKey" "<key>"   # or the AzureOpenAI__ApiKey env var
```

> A small trap on Path A: because the switch is simply *"is a key present?"*, a stale key left in user-secrets silently forces key auth. If you meant to use Entra credentials and the agent behaves as though you didn't, check your secrets before anything else.

### Step 0-2. Run it and test it in the Agents Playground

```bash
dotnet run
```

The agent listens on `http://localhost:3980`, with the channel endpoint at `/api/messages` and a plain liveness string at `GET /`.

Note the port: this starting point listens on 3980, not 3978. The two Teams starting points in this repository use 3978 and 3979, so all three agents can run side by side without colliding.

At this point there's nothing to sideload. Both of those are created for you in Phase 1, when `a365 publish` generates and stamps the manifest. What we *can* do is talk to the agent directly through the Microsoft 365 Agents Playground, which speaks the same activity protocol Teams does:
...
> The Playground works with no configuration because the starting point runs in anonymous mode — `TokenValidation.Audiences` is empty, so it accepts unauthenticated activities. The Agent 365 CLI writes the real connection settings during Phase 1. This is also the mode the warning in the introduction was about: convenient locally, not something to leave exposed on a public URL.

If you see `No connections found in configuration` in the startup logs, that's expected at this stage and not an error.
...
```bash
devtunnel host -p 3980 --allow-anonymous
```
...
> devtunnel port create my-teammate -p 3980
...
- the host started at all — if it didn't, go back to Step 2-2
...
| --- | --- |
| The host won't start | The token cache wasn't registered — Step 2-2 |
| `Failed to create authentication provider for connection name ''` | User-secrets aren't loaded in Production — Step 2-9 |
| You're prompted for a `BEARER_TOKEN` locally | You're running as `Development` — Step 2-9 |
...
| The inference span missing entirely | The backfill processor was registered too late — Step 2-3 |
...
The thing worth carrying away is how little of the hosting actually changed. It's still an ASP.NET Core app, still the Microsoft 365 Agents SDK, still `/api/messages` receiving Bot Framework activities — the same shape as any custom engine agent. What moved was the *registration*: the blueprint took over the role the Azure Bot resource used to play, and because the teammate acts as its own Agentic User, the OAuth connection that resource existed to host isn't needed either. That single substitution is what makes this scenario simpler than the Teams custom engine one in every way except the approval queue.
