...
Both are the same agent, answering the same questions from the same Microsoft Learn MCP server, and the runbook covers both. Where the SDK calls differ — and in the observability phase they differ quite a lot — the step shows each language separately.
