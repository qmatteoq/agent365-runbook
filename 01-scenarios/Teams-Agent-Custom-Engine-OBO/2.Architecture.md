...
This flow applies to both the [skill-led](3.Runbook.md) and [manual](3.Runbook-Manual.md) guides.
Registration tooling must preserve the bot's channel identity on either route.
...
| Your endpoint | ASP.NET Core, aiohttp or Express | Receives activities, runs the turn |
| A365 exporter | `Microsoft.OpenTelemetry`, `microsoft-opentelemetry` or `@microsoft/opentelemetry` | Ships spans |

### Identities, and which one goes where

This is the part people most often get wrong, so it gets a table of its own.
...
> Keep the bot app and blueprint separate. A blueprint cannot request a direct Bot Framework
> resource token in place of the bot app, and it is the wrong audience for this channel's inbound
> token. This does not prohibit the blueprint-to-child client-credentials exchange used by
> [S2S](../Service-to-Service-Agent/2.Architecture.md); that is a different identity flow.
...
The whole exchange is one call, and it looks much the same whichever stack you're on. Here it is in
Python:
...
Auto-instrumented spans, the LangChain call and the LLM call, take `gen_ai.agent.id` from
...
| Protect bot credentials during registration | `a365 setup all` can replace local channel settings with blueprint credentials. Back up the bot settings securely and restore them afterward; this does not revoke the original Entra secret |
