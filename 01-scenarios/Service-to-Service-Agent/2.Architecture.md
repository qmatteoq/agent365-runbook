...
The table uses .NET configuration names. Python and Node.js use `INGRESS_ALLOWED_CALLERS` and `INGRESS_AUDIENCE` for the same settings; the [runbook](3.Runbook.md#step-1-3-configure-and-exercise-real-token-validation) shows both formats.
...
The source brief described the child identity as having credentials and proposed choosing between an application object and a managed identity. [Agent 365 identity](https://learn.microsoft.com/microsoft-agent-365/developer/identity) describes a different model: the blueprint holds credentials, the child holds its identity and permission assignments, and a managed identity can authenticate the blueprint. We're following that model here.
...
The [runbook](3.Runbook.md) creates the inbound registrations first, then the blueprint-derived identity and its S2S exporter. That order gives us a working authorization boundary before any agent permission is granted.
